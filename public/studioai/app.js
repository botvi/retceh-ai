document.addEventListener("DOMContentLoaded", () => {
    ...
    const desktopProfileBadge = document.querySelector(
    ...
    const profileCreditsCount = document.getElementById(
    ...
    let savedImages = JSON.parse(
        localStorage.getItem("poyo_saved_images") || "[]",
    );
    let customReviews = JSON.parse(
        localStorage.getItem("poyo_custom_reviews") || "[]",
    );
    ...
    const defaultReviews = [
        {
            ...
            text: "Sangat menghemat budget! Cukup unggah foto HP, hasilnya langsung siap tayang di Instagram.",
            rating: 5,
            avatar: "B",
        },
        {
            ...
            text: "Detail kemasannya 100% akurat. Pantulan cahaya dan bayangan sangat halus seperti asli.",
            rating: 5,
            avatar: "D",
        },
        {
            ...
            text: "Proses pembuatan cepat sekali. Kualitas rasionya mantap untuk membuat banner iklan Shopee.",
            rating: 5,
            avatar: "R",
        },
        {
            ...
            text: "Latar studio estetik sekali. Warna latar belakang sangat serasi dengan warna produk hijab saya.",
            rating: 5,
            avatar: "S",
        },
        {
            ...
            text: "Pencahayaannya mantap. Sangat membantu membuat aset promosi katalog produk baru.",
            rating: 5,
            avatar: "A",
        },
    ];
        ...
        if (mobileMenuToggle && mobileMenuDrawer) {
            mobileMenuToggle.addEventListener("click", () => {
                const isCurrentlyHidden =
                    ...
                    mobileMenuDrawer.style.display === "";

                if (isCurrentlyHidden) {
                    mobileMenuDrawer.classList.remove("hidden");
                    mobileMenuDrawer.style.display = "flex";
                    if (menuToggleIcon)
                        menuToggleIcon.className = "bi bi-x text-xl";
                } else {
                    mobileMenuDrawer.style.display = "none";
                    if (menuToggleIcon)
                        menuToggleIcon.className = "bi bi-list text-xl";
                }
            });
        }
    }
    ...
    function handlePageRouting() {
        ...
        const allNavLinks = document.querySelectorAll(
            ".nav-link-landing, .nav-link-studio, .nav-link-gallery, .nav-link-topup, .nav-link-review, .nav-link-profile",
        );
        ...
        if (pageName === "index.php" || pageName === "") {
            document
                .querySelectorAll(".nav-link-landing")
                .forEach((el) => el.classList.add("active-nav"));
        } else if (pageName === "studio.php") {
            document
                .querySelectorAll(".nav-link-studio")
                .forEach((el) => el.classList.add("active-nav"));
        } else if (pageName === "gallery.php") {
            document
                .querySelectorAll(".nav-link-gallery")
                .forEach((el) => el.classList.add("active-nav"));
        } else if (pageName === "topup.php") {
            document
                .querySelectorAll(".nav-link-topup")
                .forEach((el) => el.classList.add("active-nav"));
        } else if (pageName === "review.php") {
            document
                .querySelectorAll(".nav-link-review")
                .forEach((el) => el.classList.add("active-nav"));
        } else if (pageName === "profile.php") {
            document
                .querySelectorAll(".nav-link-profile")
                .forEach((el) => el.classList.add("active-nav"));
        }
    ...
    function updateAuthStateUI() {
        if (isLoggedIn) {
            ...
            const displayName =
                localStorage.getItem("poyo_name") ||
                userEmail.split("@")[0] ||
                "User";
            ...
            const mobileAvatarImg =
                document.querySelector(".mobile-avatar-img");
            const mobileAvatarInitial = document.querySelector(
                ".mobile-avatar-initial",
            );
            const desktopAvatarImg = document.querySelector(
                ".desktop-avatar-img",
            );
            const desktopAvatarInitial = document.querySelector(
                ".desktop-avatar-initial",
            );
    ...
    function showToast(message, type = "info") {
        ...
        toast.className =
            "fixed bottom-6 right-6 bg-white dark:bg-zinc-900 border text-zinc-900 dark:text-zinc-50 rounded-lg p-4 shadow-xl z-50 flex items-center gap-3 min-w-[280px] max-w-sm transition-all duration-300";
    ...
    if (formRegister) {
        formRegister.addEventListener("submit", (e) => {
            ...
            const whatsapp = registerWhatsappInput
                ? registerWhatsappInput.value.trim()
                : "";
    ...
    function uploadFile(file, type, card, dropzone, onSuccess, onError) {
        const progressBarContainer = card.querySelector(
            ...
            if (xhr.status === 200) {
                try {
                    ...
                    if (res && res.success && res.data && res.data.file_url) {
                        ...
                        throw new Error(
                            res.msg || "Struktur respon server tidak valid",
                        );
                    }
                } catch (err) {
                    progressBarContainer.classList.add("hidden");
                ...
                try {
                    ...
                    onError(
                        errRes.error ||
                            errRes.detail ||
                            `Unggah gagal dengan status ${xhr.status}`,
                    );
                } catch (e) {
        ...
        dropzone.addEventListener("drop", (e) => {
            ...
            if (files.length > 0) {
                ...
                if (file.type.startsWith("image/")) {
                    ...
                    showToast(
                        "Harap unggah berkas gambar (PNG, JPG, WEBP)",
                        "error",
                    );
                }
            }
    ...
    if (generateBtn) {
        generateBtn.addEventListener("click", () => {
            ...
            if (credits < requiredCredits) {
                showToast(
                    `Ignis Token Anda tidak mencukupi untuk membuat desain (memerlukan ${requiredCredits} Ignis Token). Silakan isi saldo!`,
                    "error",
                );
                setTimeout(() => {
            ...
            statusDesc.textContent = "Menghubungi backend proxy retcehstudio.";
            ...
            fetch("proxy.php?action=submit", {
                    ...
                    if (!response.ok) {
                        return response.json().then((err) => {
                            throw new Error(
                                err.error ||
                                    err.detail ||
                                    "Gagal mengirim tugas ke server",
                            );
                        });
                    }
                ...
                .then((res) => {
                    if (
                        ...
                        throw new Error(
                            "Tidak ada tanda pengenal tugas yang diterima dari server",
                        );
                    }
                })
                .catch((err) => {
    ...
    function startPolling(taskId) {
        ...
        function poll() {
            ...
            if (attempts > maxAttempts) {
                ...
                showToast(
                    "Waktu pembuatan desain habis. Harap ulangi kembali.",
                    "error",
                );
                resetViewportToPlaceholder();
                return;
            ...
            fetch(`proxy.php?action=status&task_id=${taskId}`)
                .then((response) => {
                    if (response.status === 429) {
                        console.warn(
                            "retcehstudio Status Rate Limited (429). Backing off...",
                        );
                        ...
                        return response
                            ...
                            .then((err) => {
                                throw new Error(
                                    err.error ||
                                        err.detail ||
                                        `Masalah server (HTTP ${response.status})`,
                                );
                            })
                            .catch(() => {
                                throw new Error(
                                    `Masalah jaringan (HTTP ${response.status})`,
                                );
                            });
                    }
                    return response.json();
                    ...
                    if (code === 200 && data) {
                        ...
                        if (
                            ...
                            statusDesc.textContent =
                                "Menunggu antrean pembuatan di server...";
                            ...
                            if (finalUrl) {
                                setTimeout(() => {
                                    ...
                                    showToast(
                                        "Desain foto produk berhasil dibuat!",
                                        "success",
                                    );
                                }, 500);
                            } else {
                                throw new Error(
                                    "Status selesai diterima tetapi URL hasil gambar tidak ditemukan",
                                );
                            }
                        } else if (
                            rawStatus === "failed" ||
                            ...
                            throw new Error(
                                data.error_message ||
                                    data.error ||
                                    "Pembuatan desain gagal di server",
                            );
                        } else {
                        ...
                        throw new Error(
                            (data && (data.error || data.error_message)) ||
                                "Respon status tidak valid dari server",
                        );
                    }
                })
                .catch((err) => {
    ...
    if (btnEdit) {
        btnEdit.addEventListener("click", () => {
            ...
            if (generateBtn) {
                generateBtn.innerHTML =
                    '<i class="bi bi-fire"></i> Edit Desain (6 Ignis Token)';
            }
            ...
            showToast(
                "Mode Edit Aktif! Tulis instruksi perubahan di kolom kiri.",
                "info",
            );
        });
    }
    ...
    function renderGallery() {
        ...
        if (savedImages.length === 0) {
            ...
            savedImages.forEach((img) => {
                ...
                card.className =
                    "group relative rounded-xl overflow-hidden border border-zinc-200 bg-white aspect-square flex items-center justify-center p-2 shadow-sm transition-colors duration-300";
                card.innerHTML = `
            ...
            document
                .querySelectorAll(".btn-delete-gallery-img")
                .forEach((btn) => {
    ...
    if (btnClearGallery) {
        btnClearGallery.addEventListener("click", () => {
            if (
                confirm(
                    "Apakah Anda yakin ingin menghapus seluruh riwayat desain di galeri?",
                )
            ) {
                savedImages = [];
    ...
    document.querySelectorAll(".btn-purchase-credits").forEach((btn) => {
        btn.addEventListener("click", (e) => {
            ...
            e.currentTarget.innerHTML =
                '<i class="bi bi-arrow-repeat animate-spin"></i> Memproses pembayaran...';
            ...
            setTimeout(() => {
                ...
                showToast(
                    `Pembayaran berhasil! Menambahkan ${addedCredits} Ignis Token ke akun Anda.`,
                    "success",
                );
                setTimeout(() => {
                    window.location.href = "studio.php";
    ...
    function renderReviews() {
        ...
        allReviews.forEach((rev) => {
            ...
            card.className =
                "p-5 border border-zinc-200 bg-white rounded-3xl space-y-4 shadow-sm snap-center min-w-[260px] sm:min-w-[280px] max-w-[280px] flex-shrink-0 flex flex-col justify-between transition-all duration-300 hover:shadow-md hover:-translate-y-1 hover:border-zinc-300 relative group";
    ...
    function setupStarRatingSelector() {
        const starBtns =
            starRatingContainer.querySelectorAll(".star-select-btn");
        ...
        function highlightStars(val) {
            starBtns.forEach((btn) => {
                ...
                if (btnVal <= val) {
                    btn.className =
                        "bi bi-star-fill star-select-btn cursor-pointer text-amber-500 transition scale-110";
                } else {
                    btn.className =
                        "bi bi-star star-select-btn cursor-pointer text-zinc-300 dark:text-zinc-700 transition";
                }
            });
        }
    ...
    if (formReview) {
        formReview.addEventListener("submit", (e) => {
            ...
            if (selectedRating === 0) {
                showToast(
                    "Silakan pilih nilai kepuasan bintang terlebih dahulu!",
                    "error",
                );
                return;
            }
            ...
            if (starRatingContainer) {
                starRatingContainer
                    .querySelectorAll(".star-select-btn")
                    .forEach((btn) => {
                        btn.className =
                            "bi bi-star star-select-btn cursor-pointer text-zinc-300 dark:text-zinc-700 transition";
                    });
    ...
    function setupProfilePage() {
        ...
        if (profileAvatarWrapper)
            profileAvatarWrapper.addEventListener("click", triggerInput);
        if (btnUploadTrigger)
            btnUploadTrigger.addEventListener("click", triggerInput);
