                            ...
                            <div class="col-6 col-md-3">
                                <select name="status" class="form-select">
                                    ...
                                    <option value="approved" {{ request('status') == 'approved' ? 'selected' : '' }}>Aktif /
                                        Approved</option>
                                    <option value="pending" {{ request('status') == 'pending' ? 'selected' : '' }}>Tertunda
                                        / Pending</option>
                                </select>
                            </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <div class="d-flex align-items-center gap-3">
                                                ...
                                                <span class="fw-600"
                                                    style="font-size:0.85rem;">{{ $item->name ?: $item->user->name ?? '-' }}</span>
                                            </div>
                                        </td>
                                        ...
                                        <td class="text-warning">
                                            @for ($i = 1; $i <= 5; $i++)
                                                <i class="ti ti-star{{ $i <= $item->rating ? '-filled' : '' }}"
                                        ...
                                        <td>
                                            <button type="button"
                                                class="btn btn-sm btn-outline-primary d-inline-flex align-items-center gap-1 btn-lihat-pesan"
                                                data-pesan="{{ $item->pesan }}">
                                                <i class="ti ti-eye"></i> Lihat
                                            </button>
                                        </td>
                                        <td class="text-end" style="padding-right: 1.5rem;">
                                            <form action="{{ route('manage-testimoni.toggle-status', $item->id) }}"
                                                method="POST" style="display:inline-block;">
                                                @csrf
                                                <button type="submit"
                                                    class="btn btn-sm {{ $item->status == 'approved' ? 'btn-warning' : 'btn-success' }} me-1">
                                                    {{ $item->status == 'approved' ? 'Sembunyikan' : 'Setujui' }}
                                                </button>
                                            </form>
                                            <form action="{{ route('manage-testimoni.destroy', $item->id) }}"
                ...
                @if ($testimonis->hasPages())
                    <div class="card-footer d-flex align-items-center justify-content-between">
                        <small class="text-muted">
                            Menampilkan {{ $testimonis->firstItem() }}–{{ $testimonis->lastItem() }} dari
                            {{ $testimonis->total() }} ulasan
                        </small>
