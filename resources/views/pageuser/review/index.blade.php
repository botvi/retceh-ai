        ...
        <div class="rounded-3xl border border-zinc-200 bg-white p-6 shadow-xl space-y-6 transition-all duration-300 hover:shadow-2xl">
            ...
            <form action="{{ route('review.store') }}" method="POST" id="form-review" class="space-y-5">
                ...
                <div class="p-4 rounded-2xl border border-zinc-100 bg-zinc-50/50 space-y-2 text-center shadow-inner">
                    ...
                    <div class="flex justify-center gap-2.5 text-3xl text-zinc-300 select-none" id="star-rating-container">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="name" id="review-name" required value="{{ old('name', Auth::user()->name ?? '') }}" placeholder="Contoh: Budi Santoso" class="w-full bg-zinc-50 border border-zinc-200 rounded-xl p-3 text-xs focus:outline-none focus:ring-2 focus:ring-wise-green/50 focus:border-wise-green text-zinc-950 font-medium placeholder-zinc-400 transition-all">
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="role" id="review-role" required value="{{ old('role') }}" placeholder="Contoh: Pemilik Toko Kopi, Digital Marketer" class="w-full bg-zinc-50 border border-zinc-200 rounded-xl p-3 text-xs focus:outline-none focus:ring-2 focus:ring-wise-green/50 focus:border-wise-green text-zinc-950 font-medium placeholder-zinc-400 transition-all">
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea name="pesan" id="review-text" rows="3" required placeholder="Tuliskan pengalaman luar biasa Anda di sini..." class="w-full bg-zinc-50 border border-zinc-200 rounded-xl p-3 text-xs focus:outline-none focus:ring-2 focus:ring-wise-green/50 focus:border-wise-green text-zinc-950 font-medium placeholder-zinc-400 transition-all resize-none">{{ old('pesan') }}</textarea>
                </div>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function updateStars(val) {
            stars.forEach(star => {
                ...
                if (starVal <= val) {
                    star.className = 'bi bi-star-fill star-select-btn cursor-pointer transition-all duration-200 hover:scale-125 select-none text-amber-500';
                } else {
                    star.className = 'bi bi-star star-select-btn cursor-pointer transition-all duration-200 hover:scale-125 select-none text-zinc-300';
                }
            });
        }
    });
