...
@section('title', 'Profil Pengguna | retcehStudio')
...
<main class="w-full flex-grow px-4 sm:px-6 flex items-center justify-center py-5">
    ...
    <div id="view-profile" class="view-section w-full max-w-md mx-auto">
        ...
        <div class="rounded-3xl border border-zinc-200 bg-white shadow-xl overflow-hidden transition-all duration-300 hover:shadow-2xl">
            ...
            <!-- Avatar Section (Half overlapping) -->
            <form action="{{ route('profil.update') }}" method="POST" enctype="multipart/form-data" class="contents">
                @csrf
                @method('PUT')
                
                <div class="relative px-6 pb-4 flex flex-col items-center -mt-10">
                    <div class="relative group">
                        <div class="w-20 h-20 rounded-full bg-zinc-100 border-4 border-white shadow-md flex items-center justify-center overflow-hidden cursor-pointer transition-all duration-300 hover:scale-105" id="profile-avatar-wrapper" onclick="document.getElementById('profile-avatar-input').click()" title="Ganti Foto Profil">
                            @if(Auth::user()->foto_profile)
                                @php
                                    $foto = Auth::user()->foto_profile;
                                    $src = Str::startsWith($foto, ['http://', 'https://']) ? $foto : asset('uploads/foto_profile/' . $foto);
                                @endphp
                                <img class="w-full h-full object-cover" id="profile-avatar-preview" src="{{ $src }}" alt="Avatar Preview">
                            @else
                                <span class="text-2xl font-black text-forest" id="profile-avatar-initial">{{ strtoupper(substr(Auth::user()->name, 0, 1)) }}</span>
                        ...
                        <!-- Small Camera Badge -->
                        <div id="btn-upload-trigger" onclick="document.getElementById('profile-avatar-input').click()" class="absolute bottom-0 right-0 w-6 h-6 bg-wise-green hover:bg-wise-green-hover text-forest rounded-full border-2 border-white flex items-center justify-center shadow cursor-pointer transition">
                            <i class="bi bi-pencil-fill text-[9px]"></i>
                        </div>
                    </div>
                    <input type="file" name="foto_profile" id="profile-avatar-input" accept="image/jpeg,image/png,image/webp" class="hidden" onchange="previewImage(this)">
                    
                    <div class="text-center mt-3">
                        <h3 class="text-lg font-extrabold text-forest">{{ Auth::user()->name }}</h3>
                ...
                <div class="p-6 space-y-5">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="name" id="profile-name" required value="{{ old('name', Auth::user()->name) }}" class="w-full bg-zinc-50 border border-zinc-200 rounded-xl p-3 text-xs focus:outline-none focus:ring-2 focus:ring-wise-green/50 focus:border-wise-green text-zinc-950 font-medium placeholder-zinc-400 transition-all">
                    </div>

                    <!-- Input: Username -->
                    <div class="space-y-1.5">
                        <label for="profile-username" class="text-[11px] font-bold text-forest uppercase tracking-wider block flex items-center gap-1.5">
                            <i class="bi bi-at text-xs text-zinc-400"></i> Username
                        </label>
                        <input type="text" name="username" id="profile-username" required value="{{ old('username', Auth::user()->username) }}" class="w-full bg-zinc-50 border border-zinc-200 rounded-xl p-3 text-xs focus:outline-none focus:ring-2 focus:ring-wise-green/50 focus:border-wise-green text-zinc-950 font-medium placeholder-zinc-400 transition-all">
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="email" id="profile-email" readonly value="{{ Auth::user()->email }}" class="w-full bg-zinc-50/50 border border-zinc-200/50 rounded-xl p-3 text-xs text-zinc-400 cursor-not-allowed outline-none select-none font-medium">
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="tel" name="no_wa" id="profile-whatsapp" placeholder="Contoh: 081234567890" value="{{ old('no_wa', Auth::user()->no_wa) }}" class="w-full bg-zinc-50 border border-zinc-200 rounded-xl p-3 text-xs focus:outline-none focus:ring-2 focus:ring-wise-green/50 focus:border-wise-green text-zinc-950 font-medium placeholder-zinc-400 transition-all">
                    </div>
                    ...
                    <div class="p-4 rounded-2xl border border-wise-green bg-wise-green/5 flex items-center justify-between shadow-sm">
                        <div class="space-y-0.5">
                            ...
                            <div class="flex items-center gap-1">
                                ...
                                <span class="text-base font-extrabold text-forest" id="profile-credits-count">{{ Auth::user()->credits }}</span>
                                <span class="text-[10px] text-zinc-500 font-semibold uppercase tracking-wider">Gelas Kopi</span>
                            </div>
                        </div>
                        <a href="{{ route('topup.index') }}" class="inline-flex py-2 px-4 bg-forest hover:bg-forest-hover text-wise-green rounded-full font-bold text-[10px] uppercase tracking-wider transition hover:scale-105 active:scale-95 no-underline shadow">
                            Isi Saldo
                        </a>
                    </div>
