...
@section('content')
    <main class="w-full flex-grow px-4 sm:px-6 flex items-center justify-center py-5">
        ...
        <div id="view-profile" class="view-section w-full max-w-md mx-auto">
            <!-- Main Card -->
            <div
                class="rounded-3xl border border-zinc-200 bg-white shadow-xl overflow-hidden transition-all duration-300 hover:shadow-2xl">

                <!-- Beautiful Card Banner -->
                <div class="h-32 bg-gradient-to-tr from-forest to-charcoal relative flex items-end justify-center">
                    <!-- Decorative Pattern -->
                    <div
                        class="absolute inset-0 opacity-10 bg-[radial-gradient(#9fe870_1px,transparent_1px)] [background-size:16px_16px]">
                    </div>
                    <!-- Ambient glow -->
                ...
                <form action="{{ route('profil.update') }}" method="POST" enctype="multipart/form-data" class="contents">
                    ...
                    <div class="relative px-6 pb-4 flex flex-col items-center -mt-10">
                        <div class="relative group">
                            <div class="w-20 h-20 rounded-full bg-zinc-100 border-4 border-white shadow-md flex items-center justify-center overflow-hidden cursor-pointer transition-all duration-300 hover:scale-105"
                                id="profile-avatar-wrapper"
                                onclick="document.getElementById('profile-avatar-input').click()" title="Ganti Foto Profil">
                                @if (Auth::user()->foto_profile)
                                    ...
                                    @endphp
                                    <img class="w-full h-full object-cover" id="profile-avatar-preview"
                                        src="{{ $src }}" alt="Avatar Preview">
                                @else
                                    <span class="text-2xl font-black text-forest"
                                        id="profile-avatar-initial">{{ strtoupper(substr(Auth::user()->name, 0, 1)) }}</span>
                                    <img class="w-full h-full object-cover hidden" id="profile-avatar-preview"
                                        src="" alt="Avatar Preview">
                                @endif

                                <!-- Overlay on Hover -->
                                <div
                                    class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200 text-white rounded-full">
                                    <i class="bi bi-camera-fill text-lg"></i>
                                </div>
                            </div>
                            <!-- Small Camera Badge -->
                            <div id="btn-upload-trigger" onclick="document.getElementById('profile-avatar-input').click()"
                                class="absolute bottom-0 right-0 w-6 h-6 bg-wise-green hover:bg-wise-green-hover text-forest rounded-full border-2 border-white flex items-center justify-center shadow cursor-pointer transition">
                                <i class="bi bi-pencil-fill text-[9px]"></i>
                            </div>
                        </div>
                        <input type="file" name="foto_profile" id="profile-avatar-input"
                            accept="image/jpeg,image/png,image/webp" class="hidden" onchange="previewImage(this)">

                        <div class="text-center mt-3">
                            <h3 class="text-lg font-extrabold text-forest">{{ Auth::user()->name }}</h3>
                            <p class="text-[10px] uppercase font-bold tracking-widest text-zinc-400">Pengguna retcehStudio
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="p-6 space-y-5">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="email" id="profile-email" readonly value="{{ Auth::user()->email }}"
                                class="w-full bg-zinc-50/50 border border-zinc-200/50 rounded-xl p-3 text-xs text-zinc-400 cursor-not-allowed outline-none select-none font-medium">
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="tel" name="no_wa" id="profile-whatsapp" placeholder="Contoh: 081234567890"
                        ...
                        <div
                            ...
                            <div class="space-y-0.5">
                                ...
                                <div class="flex items-center gap-1">
                                    ...
                                    <span class="text-base font-extrabold text-forest"
                                        id="profile-credits-count">{{ Auth::user()->credits }}</span>
                                    <span class="text-[10px] text-zinc-500 font-semibold uppercase tracking-wider">Ignis
                                        Token</span>
                                </div>
                            </div>
                            <a href="{{ route('topup.index') }}"
                                class="inline-flex py-2 px-4 bg-forest hover:bg-forest-hover text-wise-green rounded-full font-bold text-[10px] uppercase tracking-wider transition hover:scale-105 active:scale-95 no-underline shadow">
                                Isi Saldo
                            </a>
                        </div>
