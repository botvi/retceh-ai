    ...
    <main class="w-full flex-grow px-4 sm:px-6 py-5 flex items-center justify-center">
        ...
        <div id="view-qris-checkout" class="view-section w-full max-w-md mx-auto space-y-4">
            ...
            <div class="text-center space-y-1">
                ...
                <p class="text-xs text-zinc-500 leading-relaxed">Scan QR di bawah menggunakan aplikasi e-wallet atau mobile
                    banking Anda.</p>
            </div>
            ...
            <div class="flex items-center justify-center gap-2 py-2">
                <div id="countdown-wrapper"
                    class="flex items-center gap-2 px-4 py-2 bg-amber-50 border border-amber-200 rounded-2xl text-xs font-bold text-amber-700">
                ...
                <div id="status-badge"
                    class="flex items-center gap-1.5 px-4 py-2 rounded-2xl text-xs font-bold border bg-zinc-50 border-zinc-200 text-zinc-500">
            ...
            <div class="rounded-3xl border border-zinc-200 bg-white shadow-xl overflow-hidden">
                ...
                <div class="p-6 flex flex-col items-center gap-4 border-b border-zinc-100">
                    ...
                    <div id="qris-container" class="hidden flex flex-col items-center gap-3">
                        @if (!empty($data['qris_image']))
                            ...
                            <div class="relative p-3 bg-white border-2 border-zinc-200 rounded-2xl shadow-inner">
                                <img src="{{ $data['qris_image'] }}" alt="QRIS QR Code"
                                    class="w-52 h-52 object-contain rounded-xl" id="qris-img"
                                    onerror="this.style.display='none'; document.getElementById('qris-img-fallback').style.display='block';">
                                <img src="{{ $data['qris_url'] ?? '' }}" alt="QRIS QR Code"
                                    class="w-52 h-52 object-contain rounded-xl hidden" id="qris-img-fallback">
                            </div>
                        ...
                        <p class="text-[10px] text-zinc-400 text-center">
                            Berlaku hingga <strong>{{ \Carbon\Carbon::parse($transaction->expired_at)->format('H:i') }}
                                WIB</strong>
                        </p>
                    </div>
                    ...
                    <div id="success-overlay" class="hidden flex flex-col items-center gap-3 py-4">
                        <div
                            class="w-20 h-20 rounded-full bg-emerald-50 border-2 border-emerald-200 flex items-center justify-center">
                ...
                <div class="px-5 pb-5 space-y-2.5" id="action-buttons">
                    <button id="btnPay" data-signature="{{ $data['signature'] ?? '' }}"
                    ...
                    <div class="text-center pt-1">
                        <a href="{{ route('topup.index') }}"
                            class="text-xs text-zinc-500 hover:text-zinc-900 transition underline font-semibold">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function onExpired() {
                ...
                statusBadge.className =
                    'flex items-center gap-1.5 px-4 py-2 rounded-2xl text-xs font-bold border bg-red-50 border-red-200 text-red-500';
                    ...
                    fetch(statusUrl, {
                        ...
                        .then(res => {
                            ...
                            if (status === 'SUCCESS') {
                                ...
                                showToast(
                                    'Status: Menunggu Pembayaran. Belum ada pembayaran terdeteksi.',
                                    'info');
                            }
                        })
                        .catch(err => {
