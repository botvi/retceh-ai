    ...
    <main class="w-full flex-grow px-4 sm:px-6">
        ...
        <div id="view-landing" class="view-section w-full max-w-4xl mx-auto space-y-8 py-3">
            ...
            <section class="text-center py-10 md:py-14 space-y-6 relative overflow-hidden">
                ...
                <p
                    class="text-sm sm:text-base text-zinc-500 font-normal max-w-lg mx-auto leading-relaxed transition-colors duration-300 animate-fade-in-up delay-1">
                    {{ $hero_subtitle }}
                </p>
                <div class="flex flex-col sm:flex-row items-center justify-center gap-4 animate-fade-in-up delay-2">
                    <a href="{{ route('studio.index') }}"
                        class="btn-start-generating text-sm py-3 px-8 rounded-full font-bold transition shadow-md flex items-center justify-center gap-2 no-underline">
                        Mulai Buat Desain
                    </a>
                    <a href="{{ route('topup.index') }}"
                        class="w-full sm:w-auto text-center text-sm py-3 px-8 bg-white border border-zinc-200 text-zinc-700 hover:bg-zinc-50 rounded-full font-bold transition hover:-translate-y-0.5 active:translate-y-0 shadow-sm no-underline">
                        Lihat Paket Harga
                    </a>
                </div>
            </section>
            ...
            <section
                ...
                <!-- The Deck Grid: responsive grid on desktop, horizontal scroll on mobile -->
                <div id="deck-carousel"
                    class="flex overflow-x-auto md:grid md:grid-cols-3 gap-y-24 md:gap-y-12 max-w-4xl mx-auto py-10 px-4 md:px-4 scrollbar-none snap-x snap-mandatory relative"
                    style="perspective: 1000px; scroll-behavior: smooth; -webkit-overflow-scrolling: touch;">
                    @foreach ($showcase_items as $item)
                        <!-- Product Item -->
                        <div class="snap-center shrink-0 w-[180px] sm:w-[200px] md:w-auto md:shrink carousel-deck-item group relative flex flex-col items-center select-none"
                            style="perspective: 1000px;">
                            <!-- Card Deck Wrapper -->
                            <div class="deck-wrapper relative w-[180px] h-[250px] sm:w-[200px] sm:h-[270px] transition-all duration-300 transform-style-3d cursor-pointer"
                                ...
                                <div
                                    ...
                                    <span
                                        class="absolute top-3 left-3 px-2 py-0.5 bg-zinc-900 text-white rounded-full text-[8px] font-bold tracking-wider uppercase z-20">{{ $item->label_before }}</span>
                                    <div
                                        class="w-full h-[85%] rounded-xl overflow-hidden bg-zinc-50 border border-zinc-150">
                                        <img src="{{ asset($item->image_before) }}" alt="{{ $item->title }} Asli"
                                            class="w-full h-full object-cover grayscale opacity-85 transition-all duration-300">
                                    </div>
                                    <div class="text-center py-1">
                                        <span
                                            class="text-[8px] font-black text-zinc-400 uppercase tracking-widest">{{ $item->category_label }}</span>
                                    </div>
                                </div>
                                ...
                                <div
                                    ...
                                    <span
                                        class="absolute top-3 right-3 px-2 py-0.5 bg-wise-green text-forest rounded-full text-[8px] font-bold tracking-wider uppercase z-20 shadow-sm animate-pulse">{{ $item->label_after }}</span>
                                    <div
                                        ...
                                        <img src="{{ asset($item->image_after) }}" alt="{{ $item->title }} Hasil"
            ...
            <section class="space-y-5 pt-4 pb-2">
                ...
                <div
                    class="flex overflow-x-auto gap-6 text-left pb-6 pt-2 scrollbar-none snap-x snap-mandatory -mx-4 px-4">
                    @forelse($testimonis as $testi)
                        <div
                            class="snap-center shrink-0 w-[280px] bg-white border border-zinc-200 rounded-3xl p-5 shadow-sm space-y-4 transition-all duration-300 hover:shadow-md hover:border-zinc-300">
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-10 h-10 rounded-full bg-forest text-wise-green flex items-center justify-center font-extrabold text-sm shadow-inner uppercase">
                                    {{ substr($testi->name ?? ($testi->user->name ?? 'U'), 0, 1) }}
                                </div>
                                <div class="text-left leading-tight">
                                    <h4 class="text-xs font-extrabold text-zinc-800">
                                        {{ $testi->name ?? ($testi->user->name ?? 'User') }}</h4>
                                    <span
                                        class="text-[10px] text-zinc-400 font-semibold">{{ $testi->role ?? 'Bisnis' }}</span>
                                </div>
                            </div>
                            <!-- Star Rating display -->
            ...
            <section
                ...
                <div class="relative max-w-xl mx-auto space-y-6">
                    ...
                    <p class="text-xs sm:text-sm leading-relaxed" style="color: var(--color-forest) !important;">
                        ...
                        dapatkan 50 gelas kopi gratis untuk memulai.
                    </p>
                    <div class="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
                        <a href="{{ route('studio.index') }}"
                            class="w-full sm:w-auto px-8 py-3 bg-forest text-wise-green hover:bg-forest-hover rounded-full font-bold text-xs transition uppercase tracking-wider shadow hover:-translate-y-0.5 active:translate-y-0 no-underline">
