...
<main class="w-full flex-grow px-4 sm:px-6 py-6 flex items-center justify-center">
    ...
    <div id="view-studio" class="view-section w-full max-w-5xl mx-auto space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-zinc-200/80 pb-5">
            <div class="space-y-1">
                <div class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-wise-green/10 text-forest text-[11px] font-extrabold shadow-xs border border-wise-green/30 tracking-wider uppercase">
            ...
            <div class="flex flex-wrap items-center gap-2">
                @guest
                    <a href="{{ route('login') }}" class="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-wise-green/20 hover:bg-wise-green/30 border border-wise-green/40 rounded-full text-xs font-bold text-forest transition shadow-xs no-underline">
                        <i class="bi bi-box-arrow-in-right"></i> Masuk untuk Simpan
                    </a>
                @endguest
                <div class="flex items-center gap-2 px-3.5 py-1.5 bg-white border border-zinc-200 rounded-full shadow-xs text-xs font-bold text-zinc-700">
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-6 items-stretch pt-1">
            ...
            <section class="md:col-span-7 flex flex-col gap-5">
                ...
                <!-- Product Upload Card -->
                <div class="rounded-3xl border border-zinc-200/80 bg-white p-5 flex flex-col gap-4 shadow-sm hover:shadow-md transition-all duration-300 relative overflow-hidden" id="product-upload-card">
                    <div class="flex items-center justify-between border-b border-zinc-100 pb-3">
                        <h3 class="text-xs font-black uppercase tracking-wider text-forest block flex items-center gap-2">
                            <i class="bi bi-image-fill text-wise-green"></i> Foto Produk Utama
                        </h3>
                        <span class="px-2.5 py-0.5 rounded-full bg-zinc-100 text-zinc-500 text-[10px] font-bold tracking-wider uppercase">PNG / JPG / WEBP</span>
                    </div>
                    
                    <div class="border-2 border-dashed border-zinc-200 hover:border-wise-green/90 bg-zinc-50/60 hover:bg-zinc-50 rounded-2xl p-6 flex flex-col items-center justify-center text-center cursor-pointer transition-all duration-300 relative overflow-hidden min-h-[180px] group" id="product-dropzone">
                        ...
                        <div class="dropzone-content flex flex-col items-center gap-2.5">
                            <div class="w-12 h-12 rounded-full bg-white border border-zinc-200 flex items-center justify-center text-forest group-hover:bg-wise-green group-hover:text-forest group-hover:scale-110 transition duration-300 shadow-sm">
                                <i class="bi bi-cloud-arrow-up-fill text-xl"></i>
                            </div>
                            <div class="space-y-0.5">
                                <span class="text-xs font-bold text-zinc-800 block">Unggah berkas foto produk Anda</span>
                                <span class="text-[10px] text-zinc-400 font-medium">Tarik & lepas gambar di sini (Maksimal 5MB)</span>
                            </div>
                        </div>

                        <div class="preview-container hidden absolute inset-0 w-full h-full bg-white flex items-center justify-center p-3">
                            <img src="" alt="Product Preview" class="img-preview w-full h-full object-contain rounded-xl">
                            <button type="button" class="remove-btn absolute top-3 right-3 w-8 h-8 bg-white/90 backdrop-blur border border-zinc-200 hover:bg-red-50 hover:text-red-500 hover:border-red-200 rounded-full flex items-center justify-center text-zinc-700 transition shadow-md" title="Hapus Gambar">
                                <i class="bi bi-trash3-fill text-xs"></i>
                            </button>
                        </div>
                    </div>

                    <div class="progress-bar-container hidden flex items-center gap-3 w-full mt-1" id="upload-progress-container">
                        <div class="flex-grow h-2 bg-zinc-100 rounded-full overflow-hidden relative">
                            <div class="progress-bar absolute left-0 top-0 h-full bg-wise-green rounded-full transition-all duration-300" id="upload-progress-bar" style="width: 0%;"></div>
                        </div>
                        <span class="progress-label text-[10px] font-extrabold text-forest min-w-[32px] text-right" id="upload-progress-label">0%</span>
                    </div>
                </div>

                <!-- Parameters Card -->
                <div class="rounded-3xl border border-zinc-200/80 bg-white p-5 space-y-4 shadow-sm hover:shadow-md transition-all duration-300">
                    <!-- Product Category Input (AI Suggestion) -->
                    <div class="space-y-2" id="category-section">
                        <label for="optional-note" class="text-xs font-black text-forest uppercase tracking-wider flex items-center justify-between">
                            <span class="flex items-center gap-1.5">
                                <i class="bi bi-tag-fill text-wise-green"></i> Product Category
                                <span class="text-[9px] text-zinc-400 font-semibold normal-case ml-1">(Kategori / Jenis Produk)</span>
                            </span>
                        ...
                        <div id="ai-suggestion-area" class="hidden">
                            ...
                            <div id="suggestion-loading" class="hidden items-center gap-2 py-1">
                                <span class="w-3.5 h-3.5 border-2 border-zinc-300 border-t-wise-green rounded-full animate-spin"></span>
                    ...
                    <div id="edit-instruction-container" class="hidden space-y-1.5">
                        <label for="edit-instruction" class="text-xs font-black text-forest uppercase tracking-wider block flex items-center gap-1.5">
                            <i class="bi bi-pencil-square text-amber-500"></i> Instruksi Edit / Perubahan Desain
                        </label>
                        <textarea id="edit-instruction" rows="2" placeholder="Contoh: Ubah latar belakang menjadi suasana cafe malam hari dengan pencahayaan hangat." class="w-full bg-zinc-50 border border-zinc-200 rounded-xl p-3 text-xs focus:outline-none focus:ring-2 focus:ring-wise-green/60 focus:border-wise-green text-zinc-950 font-medium placeholder-zinc-400 transition-all"></textarea>
                    </div>

                    <!-- Energetic Generate Button -->
                    <div class="pt-2">
                        <button type="button" id="generate-btn" class="w-full py-3.5 px-6 rounded-full font-black text-xs uppercase tracking-wider text-forest bg-wise-green hover:bg-wise-green-hover active:scale-[0.98] transition-all duration-200 disabled:opacity-40 disabled:pointer-events-none flex items-center justify-center gap-2 cursor-pointer shadow-md" disabled>
                            <i class="bi bi-magic"></i> Generate Desain (8 Gelas Kopi)
                        </button>
                        
                        @guest
                            <p class="text-[10px] text-zinc-400 text-center mt-2 font-medium">
                                <i class="bi bi-info-circle"></i> Anda dapat mengunggah foto untuk preview. Login diperlukan saat memproses gambar.
                            </p>
                        @endguest
                    </div>
                </div>
            ...
            <section class="md:col-span-5 flex">
                <div class="rounded-3xl border border-zinc-200/80 bg-white p-5 w-full flex flex-col justify-center items-center relative min-h-[420px] shadow-sm hover:shadow-md transition-all duration-300" id="viewport-card">
                    
                    <!-- Placeholder state -->
                    <div class="text-center max-w-xs flex flex-col items-center gap-4" id="viewport-placeholder">
                        <div class="w-14 h-14 rounded-full bg-zinc-50 border border-zinc-200/80 flex items-center justify-center text-zinc-400 shadow-inner">
                            <i class="bi bi-card-image text-2xl"></i>
                        </div>
                        <div class="space-y-1.5">
                            <h3 class="text-xs font-black text-forest uppercase tracking-wider">Area Hasil AI Studio</h3>
                            <p class="text-[11px] text-zinc-450 leading-relaxed font-medium">
                                Unggah foto produk Anda, tulis kategori opsional, lalu klik **Generate Desain** untuk meluncurkan AI.
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="hidden flex flex-col items-center justify-center gap-6 w-full max-w-xs" id="viewport-loading">
                        ...
                        <div class="text-center space-y-1.5 w-full">
                            <h3 id="status-heading" class="text-xs font-black text-forest uppercase tracking-wider">Mengirimkan Tugas...</h3>
                            <p id="status-desc" class="text-[10px] text-zinc-450 leading-relaxed w-full break-words font-medium"></p>
                        </div>
                    ...
                    <div class="hidden w-full h-full flex flex-col gap-4" id="viewport-result">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-2.5 w-full">
                            <a href="" target="_blank" id="btn-download" class="inline-flex items-center justify-center gap-1.5 px-3 py-2.5 bg-forest hover:bg-forest-hover text-wise-green rounded-full text-[11px] font-black transition shadow-xs no-underline uppercase tracking-wider text-center" download="product-design.jpg">
                                <i class="bi bi-download"></i> Unduh
                            </a>
                            <button type="button" id="btn-save-to-gallery" class="inline-flex items-center justify-center gap-1.5 px-3 py-2.5 bg-wise-green hover:bg-wise-green-hover text-forest rounded-full text-[11px] font-black transition shadow-xs uppercase tracking-wider">
                                <i class="bi bi-bookmark-fill"></i> Simpan
                            </button>
                            <button type="button" id="btn-edit" class="inline-flex items-center justify-center gap-1.5 px-3 py-2.5 bg-white border border-zinc-200 hover:bg-zinc-50 text-zinc-700 rounded-full text-[11px] font-bold transition shadow-xs uppercase tracking-wider">
...
<style>
    ...
    .suggestion-chip:nth-child(1)  { animation-delay: 0.00s; }
    ...
    .suggestion-chip:nth-child(7)  { animation-delay: 0.30s; }
    .suggestion-chip:nth-child(8)  { animation-delay: 0.35s; }
</style>
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        async function fetchAISuggestions(imageUrl) {
            ...
            try {
                ...
                suggestionChips.innerHTML = `<span class="text-[10px] text-zinc-400 italic">Suggestion tidak tersedia saat ini.</span>`;
            } finally {
        ...
        function renderChips(suggestions) {
            ...
            suggestions.forEach(label => {
                ...
                chip.addEventListener('click', () => {
                    ...
                    if (optionalNoteInput.value === label && chip.classList.contains('active')) {
        ...
        optionalNoteInput.addEventListener('input', () => {
            if (optionalNoteInput.value.trim()) {
                ...
                suggestionChips.querySelectorAll('.suggestion-chip.active').forEach(c => c.classList.remove('active'));
            } else {
                clearCategoryBtn.classList.add('hidden');
            }
        });
                ...
                if (xhr.status === 200) {
                    try {
                        ...
                        if (res && res.success && res.file_url) {
                            ...
                            reader.onload = (event) => {
                                dropzone.querySelector('.preview-container img').src = event.target.result;
        ...
        productDropzone.addEventListener('drop', (e) => {
            ...
            if (files && files.length > 0) {
                ...
                uploadFile(file, productCard, productDropzone, 
                    ...
                    (errorMsg) => { showToast(errorMsg, 'error'); resetDropzone(); }
                );
            }
        });
        ...
        generateBtn.addEventListener('click', () => {
            ...
            if (credits < requiredCredits) {
                showToast(`Gelas kopi Anda tidak mencukupi (memerlukan ${requiredCredits} gelas kopi).`, 'error');
                setTimeout(() => {
            ...
            .then(res => {
                if (res && res.success && res.task_id) {
                    ...
                    const counts = document.querySelectorAll('.desktop-credit-count, .mobile-credit-count');
                    ...
                    statusDesc.innerHTML = `Tugas ID: ${res.task_id}<br><span style="font-size:0.8rem; display:block; margin-top:5px; opacity:0.8;">Produk Teridentifikasi: <strong>${res.product_desc || 'Produk utama'}</strong></span>`;
                ...
                .then(res => {
                    if (res && res.success) {
                        ...
                        if (status === 'finished' || status === 'succeed' || status === 'success' || status === 'completed') {
                            ...
                            statusHeading.textContent = status === 'pending' || status === 'queued' ? 'Dalam Antrean' : 'Memproses Desain';
                            statusDesc.textContent = status === 'pending' || status === 'queued' ? 'Menunggu antrean di server...' : `Membuat latar belakang realistik (Kemajuan: ${fakeProgress}%).`;
        ...
        btnEdit.addEventListener('click', () => {
            ...
            document.getElementById('product-upload-card').scrollIntoView({ behavior: 'smooth' });
