...
<head>
    ...
    <meta name="description" content="Ubah foto produk Anda menjadi foto iklan premium secara instan dengan kecerdasan AI.">
    
    <!-- Google Fonts: Inter, Outfit, Plus Jakarta Sans -->
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&family=Outfit:wght@300;400;500;600;700;800&family=Plus+Jakarta+Sans:wght@300;400;500;600;700;800&family=Syne:wght@750;800&display=swap" rel="stylesheet">
    ...
    <header class="w-full border-b border-zinc-200 bg-white sticky top-0 z-40 mb-4 shadow-sm">
        <div class="w-full max-w-6xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
            <div class="flex items-center gap-8">
                <!-- Brand Logo -->
                <a href="{{ route('index') }}" id="nav-logo" class="flex items-center cursor-pointer font-bold text-zinc-900 no-underline">
                    <span class="tracking-tighter text-lg font-black text-forest lowercase">retche</span>
                    <span class="relative inline-block text-wise-green bg-forest px-1.5 py-0.5 rounded-lg tracking-tighter text-lg font-black lowercase overflow-hidden text-left align-middle whitespace-nowrap">
                ...
                <nav class="hidden md:flex items-center gap-6 text-sm font-semibold text-zinc-500">
                    <a href="{{ route('index') }}" class="nav-link-landing {{ Route::is('index') ? 'active-nav' : '' }}">Beranda</a>
                    <a href="{{ route('studio.index') }}" class="nav-link-studio {{ Route::is('studio.index') ? 'active-nav' : '' }}">Studio</a>
                    <a href="{{ route('gallery.index') }}" class="nav-link-gallery {{ Route::is('gallery.index') ? 'active-nav' : '' }}">Galeri</a>
                    <a href="{{ route('topup.index') }}" class="nav-link-topup {{ Route::is('topup.index') ? 'active-nav' : '' }}">Top Up</a>
                </nav>
            </div>
            ...
            <div class="flex items-center gap-4">
                @auth
                    ...
                    <div class="items-center gap-3 hidden md:flex">
                        <a href="{{ route('topup.index') }}" class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-zinc-100 border border-zinc-200 text-xs font-semibold text-zinc-700 hover:bg-zinc-200 transition no-underline">
                            <i class="bi bi-cup-hot-fill text-amber-700"></i>
                            <span>{{ Auth::user()->credits }}</span> Gelas Kopi
                        </a>
                        <a href="{{ route('profil') }}" class="flex items-center gap-2 text-zinc-700 hover:text-zinc-900 no-underline" title="Profil Anda">
                            <div class="w-7 h-7 rounded-full bg-zinc-200 border border-zinc-200 flex items-center justify-center overflow-hidden flex-shrink-0">
                                @if(Auth::user()->foto_profile)
                                    @php
                                        $foto = Auth::user()->foto_profile;
                                        $src = Str::startsWith($foto, ['http://', 'https://']) ? $foto : asset('uploads/foto_profile/' . $foto);
                                    @endphp
                                    <img class="w-full h-full object-cover" src="{{ $src }}" alt="Avatar">
                                @else
                                    <span class="text-[11px] font-bold text-zinc-700">{{ strtoupper(substr(Auth::user()->name, 0, 1)) }}</span>
                                @endif
                            </div>
                            <span class="text-xs font-semibold">{{ Auth::user()->name }}</span>
                        </a>
                        <a href="{{ route('logout') }}" class="btn-logout text-xs py-1.5 px-3 border border-zinc-200 rounded-full hover:bg-zinc-150 transition cursor-pointer no-underline">Keluar</a>
                    </div>
                @else
                    <!-- Desktop Login / Signup Actions (Logged Out) -->
                    <a href="{{ route('login') }}" class="btn-nav-login hidden md:inline-flex text-xs font-bold text-forest py-2 px-4 rounded-full hover:bg-zinc-100 transition no-underline">Masuk</a>
                    <a href="{{ route('register') }}" class="btn-nav-login hidden md:inline-flex text-xs py-2 px-5 bg-wise-green text-forest rounded-full font-bold transition hover:bg-wise-green-hover cursor-pointer no-underline">Daftar</a>
                @endauth

                <!-- Mobile Hamburger Menu Trigger -->
                <button type="button" id="mobile-menu-toggle" class="md:hidden flex items-center justify-center w-8 h-8 text-forest hover:bg-zinc-100 rounded-full transition cursor-pointer" title="Menu">
        ...
        <div id="mobile-menu-drawer" class="hidden md:hidden w-full border-t border-zinc-200 bg-white px-6 py-5 flex flex-col gap-4 shadow-inner">
            <nav class="flex flex-col gap-3 font-semibold text-sm text-zinc-500">
                <a href="{{ route('index') }}" class="py-1 hover:text-zinc-900 transition {{ Route::is('index') ? 'text-forest font-bold' : '' }}">Beranda</a>
                <a href="{{ route('studio.index') }}" class="py-1 hover:text-zinc-900 transition {{ Route::is('studio.index') ? 'text-forest font-bold' : '' }}">Studio</a>
                <a href="{{ route('gallery.index') }}" class="py-1 hover:text-zinc-900 transition {{ Route::is('gallery.index') ? 'text-forest font-bold' : '' }}">Galeri</a>
                <a href="{{ route('topup.index') }}" class="py-1 hover:text-zinc-900 transition {{ Route::is('topup.index') ? 'text-forest font-bold' : '' }}">Top Up</a>
            </nav>
            ...
            @auth
                ...
                <div class="flex flex-col gap-3 pt-3 border-t border-zinc-150">
                    <div class="flex items-center justify-between">
                        <a href="{{ route('profil') }}" class="flex items-center gap-2.5 text-zinc-700 hover:text-zinc-900 no-underline" title="Profil Anda">
                            <div class="w-8 h-8 rounded-full bg-zinc-200 border border-zinc-200 flex items-center justify-center overflow-hidden flex-shrink-0">
                                @if(Auth::user()->foto_profile)
                                    <img class="w-full h-full object-cover" src="{{ Str::startsWith(Auth::user()->foto_profile, ['http://', 'https://']) ? Auth::user()->foto_profile : asset('uploads/foto_profile/' . Auth::user()->foto_profile) }}" alt="Avatar">
                                @else
                                    <span class="text-[12px] font-bold text-zinc-700">{{ strtoupper(substr(Auth::user()->name, 0, 1)) }}</span>
                                @endif
                            </div>
                    ...
                    <a href="{{ route('logout') }}" class="btn-logout w-full text-center text-xs py-2 border border-zinc-200 rounded-full hover:bg-zinc-100 transition cursor-pointer font-bold text-forest mt-1 no-underline">Keluar</a>
                </div>
            @else
                <!-- Mobile Login / Signup Actions (Logged Out) -->
                <div class="flex flex-col gap-2 pt-3 border-t border-zinc-150">
                    <a href="{{ route('login') }}" class="w-full text-center text-sm font-bold text-forest py-2.5 border border-zinc-200 rounded-full hover:bg-zinc-100 transition no-underline">Masuk</a>
                    <a href="{{ route('register') }}" class="w-full text-center text-sm py-2.5 bg-wise-green text-forest rounded-full font-bold transition hover:bg-wise-green-hover cursor-pointer no-underline">Daftar</a>
                </div>
            @endauth
        </div>
    </header>
