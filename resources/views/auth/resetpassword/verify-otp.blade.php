...
<head>
    ...
    <title>Verifikasi OTP - retcehstudio</title>
    ...
    <div id="view-verify-otp" class="w-full max-w-sm">
        <div
            ...
            <div class="text-center space-y-1.5">
                <div
                    class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-800 text-[11px] font-bold shadow-sm border border-emerald-500/20">
                ...
                <p class="text-xs text-zinc-550 leading-relaxed">Masukkan 6 digit kode verifikasi OTP yang telah dikirim
                    ke nomor WhatsApp Anda <b>{{ $no_wa }}</b>.</p>
            </div>
            ...
            <form action="{{ route('forgot-password.verify-otp') }}" method="POST" class="space-y-4">
                ...
                <div class="space-y-2">
                    <label for="otp"
                        class="text-[11px] font-bold text-forest uppercase tracking-wider block text-center">
                        Masukkan Kode OTP Anda
                    </label>
                    <input type="text" name="otp" id="otp" required placeholder="••••••" maxlength="6"
            ...
            <div class="text-center pt-2 border-t border-zinc-100 flex flex-col gap-1.5 text-xs text-zinc-500">
                ...
                <a href="{{ route('forgot-password') }}"
                    class="text-zinc-800 hover:text-zinc-900 transition underline font-semibold">Request OTP Baru</a>
            </div>
        </div>
    </div>
